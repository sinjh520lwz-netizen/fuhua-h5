...
"""
JH策略 v4 — 低频均值回归版
核心改变：每周最多1笔交易，持仓10-15天，大幅降低交易成本
"""
...
TRADE_COOLDOWN = 5  # 每5个交易日最多开1笔新仓
...
def score_v4(kline_dicts, kidx):
    ...
    if rsi > 40:
        return 0, {'硬过滤': f'RSI{rsi:.0f}>40'}
    
    # 2. 5日跌幅（必须跌过）
    if len(closes) >= 6:
        mom5 = (closes[-1] / closes[-6] - 1) * 100
        if mom5 > -2:
            return 0, {'硬过滤': f'5日仅跌{mom5:.1f}%'}
    else:
        return 0, {}
    ...
    if down_days >= 5:
        score += 10
        factors[f'连跌{down_days}天'] = 10
    elif down_days >= 3:
        score += 6
        factors[f'连跌{down_days}天'] = 6
    ...
    if len(closes) >= 20:
        ...
        if deviation < -10:
            score += 8
            factors[f'偏离MA20{deviation:.1f}%'] = 8
        elif deviation < -6:
            score += 5
            factors[f'偏离MA20{deviation:.1f}%'] = 5
    
    return max(score, 0), factors

def precompute_v4(all_klines, trading_dates):
    cache_file = os.path.join(DATA_DIR, 'v4_score_cache.json')
    if os.path.exists(cache_file):
        try:
            with open(cache_file) as f:
                cache = json.load(f)
            if len(cache) > 0:
                print(f"  📦 使用v4缓存 ({len(cache)}只)")
                return cache
        except:
            pass
    ...
    for idx, (code, info) in enumerate(all_klines.items()):
        ...
        for td in trading_dates:
            ...
            score, factors = score_v4(kline_dicts, kidx)
            if score >= 15:
...
def run_backtest_v4(score_cache, all_klines, trading_dates, test_dates_set, params):
    tp_pct = params['tp']
    sl_pct = params['sl']
    ...
    for di, date in enumerate(test_dates):
        ...
        # 2. 低频开仓：冷却期内不开新仓
        if di - last_trade_di < TRADE_COOLDOWN:
            continue
        ...
        for code, date_scores in score_cache.items():
            ...
            if code in holdings:
                continue
            if code in recent_trades and di - recent_trades[code] < 15:
                continue
            sd = date_scores[date]
...
def run_cpcv_v4():
    ...
    print(f"📋 规则: 每{TRADE_COOLDOWN}天最多1笔, 最多{TOP_N}只, 持仓≤{MAX_HOLD}天")
    
    score_cache = precompute_v4(all_klines, trading_dates)
    ...
    param_grid = [
        ...
        {'tp': 10, 'sl': 5, 'min_score': 15},
        {'tp': 10, 'sl': 6, 'min_score': 15},
        {'tp': 8, 'sl': 5, 'min_score': 20},
        {'tp': 8, 'sl': 6, 'min_score': 20},
        {'tp': 10, 'sl': 6, 'min_score': 20},
    ]
    ...
    print(f"📊 {len(param_grid)}组 × {len(combinations)}组合 = {len(param_grid)*len(combinations)}次\n")
    ...
    for pi, params in enumerate(param_grid):
        ...
        for test_fold_indices in combinations:
            ...
            test_trades, final_capital = run_backtest_v4(
        ...
        label = f"TP+{params['tp']}% SL-{params['sl']}% ≥{params['min_score']}"
        print(f"  [{pi+1}/{len(param_grid)}] {label} → {robustness:.0%} ({profitable_count}/{len(combinations)}) 平均{avg_return:+.1f}%")
    ...
    print(f"\n{'='*65}")
    print(f"🔬 v4 CPCV 最优 (耗时{elapsed:.0f}s)")
    print(f"{'='*65}")
    print(f"  参数: TP+{best_params['tp']}% SL-{best_params['sl']}% ≥{best_params['min_score']}")
    ...
    # 完整回测
    print(f"\n📊 完整回测...")
    full_trades, full_capital = run_backtest_v4(
    ...
    if n > 0:
        ...
        print(f"  交易: {n}笔 (约{n/(n_dates/5):.1f}笔/周)")
    ...
    output = {
        'strategy': 'JH v4 低频均值回归',
        'version': '4.0',
        'core_change': '每周最多1笔 + 持仓12天 + 极度超卖才买',
        'improvements': [
            '交易频率降至每周1笔（从每天0.7笔）',
            '持仓延长至12天（从7天）',
            '同时持仓减至2只（从5只）',
            '信号更严格：RSI<40 + 5日跌>2% + 连跌3天',
            '完整交易成本',
            'CPCV验证',
        ],
        'params': {
    ...
    out_file = os.path.join(DATA_DIR, 'v4_results.json')
